...
N = int(input()); R = []; Z = [SortedList(l1=512, l2=128) for _ in range(N+1)]; S = [*range(N+1)]; P = [*range(N)]
for i in range(N): r, m = map(int, input().split()); R.append(r); P[i] = m-1
for i in range(N-1, -1, -1):
    k = len(z:=Z[s:=S[i]]); p = z.index(r:=R[i])
    if p < k: z.pop(p); k -= 1
    z.append(r); k += 1
    if k < (l:=len(u:=Z[S[P[i]]])):
        while k: k -= 1; u.append(z.pop())
    else:
        while l: l -= 1; z.append(u.pop())
        S[P[i]] = S[i]
print(len(Z[S[0]]))