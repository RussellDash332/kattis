import sys; from math import *; from bisect import *; n = -1; M = 10**9+7
class SortedList:
    def __init__(s, A=None, l1=200, l2=64, mv=~(1<<31)):
...
def cpp(p):
    p.sort(); best = (1e38, None, None); j = 0; n = len(p); s = SortedList()
    for i in range(n):
        d = ceil(best[0]**.5); x, y = p[i]//M, p[i]%M
        while j < n and x-p[j]//M >= d: s.remove(p[j]%M*M+p[j]//M); j += 1
        b = s.index((y-d)*M+x)
        for k in range(b, min(b+5, len(s))):
            e = s[k]; new = (x-e%M)**2+(y-e//M)**2
            if new < best[0]: best = (new, (x, y), (e//M, e%M))
        s.append(y*M+x)
    _, (x1, y1), (y2, x2) = best
    sys.stdout.write(f'{x1/100-10**5} {y1/100-10**5} {x2/100-10**5} {y2/100-10**5}\n')
for l in sys.stdin:
    if n == -1: n, p = int(l), []
    else:
        x, y = map(float, l.split()); p.append(int(100*x+10**7)*M+int(100*y+10**7)); n -= 1
        if n == 0: cpp(p); n = -1