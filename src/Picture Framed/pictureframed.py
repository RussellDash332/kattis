from bisect import*;import sys;input=sys.stdin.readline
class SortedList:
    def __init__(s, A=None, l1=512, l2=128, mv=~(1<<30)):
...
N,R,C=I();P=sorted(I()for _ in'.'*N)
x=0;a=1;s=SortedList()
while a<=R and(x<N or len(s)):
 if not s:a=P[x][0]
 while x<N and P[x][0]==a:s.append(P[x][1]);x+=1
 b=1
 while len(s)and b<=C:
  q=s.index(b+1)-1
  if~q:s.pop(q)
  ...
  b+=1
 a+=1
print('CUHHE EOSHE.!.!.!'[bool(x<N or s)::2])