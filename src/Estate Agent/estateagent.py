...
n, m = map(int, input().split()); C = [[10**(9-3*(i==j)) for j in range(n)] for i in range(n)]
for _ in  range(m): f, h, a = map(int, input().split()); C[f-1][h-1] = 10**6-a
print((10**6*n-lapjv(C))/20)