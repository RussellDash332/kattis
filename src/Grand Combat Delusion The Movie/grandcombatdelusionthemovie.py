# /grandcombatdelusion
Z=10**18;I=lambda:[*map(int,input().split())];n,m=I();h,d=I();c=I();s=I();f=lambda x:sum(~-i//(d+x)if d+x>0else Z for i in s)<h
...
from bisect import *
lo, hi = sum(x for x in c if x<0)-67, sum(x for x in c if x>0)+67
while lo < hi:
    if f(mi:=(lo+hi)//2): hi = mi
    else: lo = mi+1
if f(0): Z = min(Z, 0)
Q = SortedList([0], l1=512, l2=128); p = 0
for x in c:
    p += x; q = Q.index(p-lo+1)
    if q > 0: Z = min(Z, p-Q[q-1])
    Q.append(p)
y = z = 0
for x in c: z = min(z, y:=min(x, y+x))
if Z > 10**17: print('Of erfitt!')
elif Z == min(z, 0): print('Of audvelt!')
else: print(Z, 'er matulegt')