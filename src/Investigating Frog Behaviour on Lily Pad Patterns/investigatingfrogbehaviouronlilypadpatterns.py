import os, io, sys; input = io.BytesIO(os.read(0, os.fstat(0).st_size)).readline; from array import *; from bisect import *
class SortedList:
    def __init__(s, A=None, l1=200, l2=64, mv=~(1<<30)):
        ...
        s._ex()
    def clear(s): s.b = [[]]; s.bc = array('i', [0]); s.bm = array('i', [0]); s.sm = array('i'); s.ss = array('i'); s.cnt = 0
...
s = {*a}
q = int(input())
b = SortedList([i for i in range(1, max(a)+q+1) if i not in s])
for _ in range(q): x = int(input())-1; p = b.index((v:=a[x])+1); a[x] = b.pop(p); b.append(v); print(a[x])